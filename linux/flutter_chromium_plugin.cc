...
#include "flutter_chromium_texture.h"

#define FLUTTER_CHROMIUM_PLUGIN(obj)                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), flutter_chromium_plugin_get_type(), \
                              FlutterChromiumPlugin))

struct _FlutterChromiumPlugin
{
...
G_DEFINE_TYPE(FlutterChromiumPlugin, flutter_chromium_plugin, g_object_get_type())
...
static void flutter_chromium_plugin_handle_method_call(
  FlutterChromiumPlugin *self,
...
static void flutter_chromium_plugin_dispose(GObject *object)
{
  // Remove this window's plugin instance from the global registry first.
  webviewPlugins.erase(FLUTTER_CHROMIUM_PLUGIN(object)->m_window);
  // Explicitly release the shared_ptr to break cycles before GObject teardown.
  FLUTTER_CHROMIUM_PLUGIN(object)->m_plugin.reset();
  // CEF shutdown is handled explicitly via 'quit' or by the atexit handler.
  G_OBJECT_CLASS(flutter_chromium_plugin_parent_class)->dispose(object);
}

static void flutter_chromium_plugin_class_init(FlutterChromiumPluginClass *klass)
{
  G_OBJECT_CLASS(klass)->dispose = flutter_chromium_plugin_dispose;
}

static void flutter_chromium_plugin_init(FlutterChromiumPlugin *self)
{
...
static void method_call_cb(FlMethodChannel *channel, FlMethodCall *method_call,
                           gpointer user_data)
{
  FlutterChromiumPlugin *plugin = FLUTTER_CHROMIUM_PLUGIN(user_data);
  flutter_chromium_plugin_handle_method_call(plugin, method_call);
}

void flutter_chromium_plugin_register_with_registrar(FlPluginRegistrar *registrar)
{
  FlutterChromiumPlugin *plugin = FLUTTER_CHROMIUM_PLUGIN(
      g_object_new(flutter_chromium_plugin_get_type(), nullptr));
